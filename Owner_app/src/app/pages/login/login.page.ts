...
export class LoginPage implements OnInit {
  ...
  onLogin(form: NgForm) {
    ...
    if (form.valid) {
      ...
      this.api.post_public('v1/auth/store_login', this.login).then((data: any) => {
        this.isLogin = false;
        console.log(data);
        if (data && data.status && data.status == 200 && data.user && data.user.type == 'freelancer') {
          localStorage.setItem('uid', data.user.id);
          localStorage.setItem('token', data.token);
          localStorage.setItem('name', data.store.name);
          localStorage.setItem('cover', data.store.cover);
          localStorage.setItem('address', data.store.address);
          this.updateFCMToken();
          this.util.navigateRoot('tabs');
        } else if (data && data.status == 401 && data.error.error) {
          this.util.errorToast(data.error.error, 'danger');
        } else if (data && data.user && data.user.type != 'freelancer') {
