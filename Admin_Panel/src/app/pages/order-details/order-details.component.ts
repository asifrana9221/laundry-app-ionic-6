...
export class OrderDetailsComponent implements OnInit {
  ...
  paymentRef: any;
  orderStatusNames = [
    this.util.translate('created'),
    this.util.translate('accepted'),
    this.util.translate('rejected'),
    this.util.translate('ongoing'),
    this.util.translate('completed'),
    this.util.translate('cancelled'),
    this.util.translate('refunded'),
    this.util.translate('delayed'),
    this.util.translate('awaiting payment')
  ];
  payment = [
  ...
  getOrder() {
    ...
    this.api.post_private('v1/orders/getByIdAdmin', param).then((data: any) => {
      ...
      if (data && data.status && data.status == 200 && data.data) {
        ...
        this.orderAt = info.order_to == 1 ? 'Home' : 'Self Pickup';
  ...
  printOrder() {
    window.open(this.api.baseUrl + 'v1/orders/getOrderInvoice?id=' + this.id + '&token=' + localStorage.getItem('token'), '_system');
  }
